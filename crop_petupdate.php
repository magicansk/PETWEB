...
function GetSQLValueStringUP($theValue, $theType)
	{
		switch ($theType) {
			case "string":
	...
	if (isset($_POST["action"]) && ($_POST["action"] == "update")) {
		...
		$stmt->bind_param(
			"ssssisi",
			GetSQLValueStringUP($_POST["p_name"], 'string'),
			GetSQLValueStringUP($_POST["p_class"], 'string'),
			GetSQLValueStringUP($_POST["p_breed"], 'string'),
			GetSQLValueStringUP($_POST["p_sex"], 'string'),
			GetSQLValueStringUP($_POST["p_age"], 'int'),
			GetSQLValueStringUP($_POST["p_introduction"], 'string'),
			GetSQLValueStringUP($_POST["p_id"], 'int')
		);
		$stmt->execute();
		$p_id = $_POST["p_id"];
		$stmt->close();
		...
		for ($i = 0; $i < count($_FILES["c_pp_picurl"]["name"]); $i++) {
			if ($_FILES["c_pp_picurl"]["tmp_name"][$i] != "") {
				...
				$stmt->bind_param(
					"s",
					GetSQLValueStringUP($p_id . $rand . $_FILES["c_pp_picurl"]["name"][$i], "string")
				);
				$stmt->execute();
		...
		for ($i = 0; $i < count($_FILES["c_pp_picurl"]); $i++) {
			if ($_POST["c_pp_subject"][$i] != "") {
				...
				$stmt->bind_param(
					"s",
					GetSQLValueStringUP($_POST["c_pp_subject"][$i], "string")
				);
				$stmt->execute();
				$stmt->close();
			}
		...
		for ($i = 0; $i < count($_FILES["pp_picurl"]["name"]); $i++) {
			if ($_FILES["pp_picurl"]["tmp_name"][$i] != "") {
				...
				$stmt->bind_param(
					"iss",
					GetSQLValueStringUP($p_id, "int"),
					GetSQLValueStringUP($p_id . $rand . $_FILES["pp_picurl"]["name"][$i], "string"),
					GetSQLValueStringUP($_POST["pp_subject"][$i], "string")
				);
				$stmt->execute();
		...
		//重新導向
		header("Location: pet_update.php?id=" . $_GET['id']);
	}
?>